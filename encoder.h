/**
   A file to hold the wheel encoder routines
...
/**
  This ISR handles just Encoder 0  ISR to read the Encoder1 
  Channel A and B pins and then look up based on  transition
  what kind of rotation must have occured.
...
ISR( INT6_vect ) {
  ...
  if ( state == 1 ) {
    right_wheel_count_e1 = right_wheel_count_e1 - 1; // ccw
  } else if ( state == 2 ) {
    right_wheel_count_e1 = right_wheel_count_e1 + 1;  // cw
  } else if ( state == 4 ) {
    right_wheel_count_e1 = right_wheel_count_e1 + 1;  // cw
  } else if ( state == 7 ) {
    right_wheel_count_e1 = right_wheel_count_e1 - 1;  // ccw
...
/**
  This ISR handles just Encoder 0  ISR to read the Encoder0 
  Channel A and B pins and then look up based on  transition
  what kind of rotation must have occured.
...
ISR( PCINT0_vect ) {
  ...
  // against the lecture slides.
  if ( state == 1 ) {
    left_wheel_count_e0 = left_wheel_count_e0 - 1;  // ccw
  } else if ( state == 2 ) {
    left_wheel_count_e0 = left_wheel_count_e0 + 1;  // cw
  } else if ( state == 4 ) {
    left_wheel_count_e0 = left_wheel_count_e0 + 1;  // cw
  } else if ( state == 7 ) {
    left_wheel_count_e0 = left_wheel_count_e0 - 1;  // ccw
...
/**
  This setup routine enables interrupts for encoder0, right wheel.
  The interrupt is automatically triggered when one of the encoder 
  pin changes.This is really convenient! It means we don't have to 
  check the encoder manually.
...
void setupEncoder1() {
  ...
  // Initialise our counA & B signals
  // to zero, we don't know what they were.
  oldE1_A = 0;
  oldE1_B = 0;

  right_wheel_count_e1 = 0;
...
/**
  This setup routine enables interrupts for encoder0, left wheel.
  The interrupt is automatically triggered when one of the encoder 
  pin changes.This is really convenient! It means we don't have to 
  check the encoder manually.
